<template>
  ...
  <UiParentCard title="출하지시서 등록">
    <v-row class="mb-4">
      ...
      <v-col cols="6">
        <v-select label="창고번호" v-model="form.wrNo" dense outlined />
      </v-col>
      <v-row justify="end">
        <v-btn color="warning" class="mr-3" @click="openModal2">입고 조회</v-btn>
        <template #append-inner>
          <i
            class="fa-solid fa-magnifying-glass"
            style="cursor: pointer; font-size: large; margin-right: 0.5rem"
            @click="openModal1('제품 규격 조회', materialRowData1, materialColDefs1)"
          ></i>
        </template>
...
const materialColDefs1 = [
  { field: '입고번호', headerName: '입고번호', flex: 1 },
...
const openModal1 = async (title) => {
  ...
  modalColDefs1.value = materialColDefs1;
